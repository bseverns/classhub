...
| Teacher/admin (bounded remote helper control) | `/teach/class/<id>` | Request or stop remote helper compute for a live class window, inspect state/expiry, and export class-scoped evidence without exposing provider controls to students | `11-invite-only-enrollment.png`, `08-health-checks-terminal.png` |
