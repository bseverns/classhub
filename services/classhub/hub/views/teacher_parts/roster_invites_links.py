"""Teacher class invite-link endpoints."""
...
def teach_disable_invite_link(request, class_id: int):
    ...
    try:
        ...
        invite_id = 0
    if invite_id <= 0:
        return _class_notice_redirect(request, classroom, error="Invalid invite link selection.")

    invite = ClassInviteLink.objects.filter(id=invite_id, classroom=classroom).first()
    if invite is None:
        return _class_notice_redirect(request, classroom, error="Invite link not found.")
    ...
    return _class_notice_redirect(request, classroom, notice="Invite link disabled.")
