"""Internal row-builder helpers for facilitator support snapshots."""

from __future__ import annotations

from ...models import Material, Module, StudentEvent, StudentIdentity
from ..telemetry_reads import student_events_queryset
from .shared import detail_int


def build_module_material_lookups(*, classroom, modules: list[Module]) -> tuple[dict[int, str], dict[int, dict]]:
    ...
    return module_titles, material_lookup
...
def build_stuck_rows(
    *,
    ...
    activity_events = (
        ...
        .only("student_id", "event_type", "details", "created_at")
    ...
    for student_id, stuck_event in latest_stuck_by_student.items():
        ...
        if (
            latest_signal
            and latest_signal.created_at > stuck_event.created_at
            and latest_signal.event_type != StudentEvent.EVENT_MICRO_CHECK_STUCK
        ):
            continue
    ...
    rows.sort(key=lambda row: (-int(row["waiting_minutes"]), str(row["display_name"]).lower()))
    return rows


def build_delete_request_rows(*, classroom_id: int, now, students: list[StudentIdentity]) -> list[dict]:
    student_by_id = {int(student.id): student for student in students}
    ...
    for event in delete_request_events:
        ...
        if student_id <= 0:
            continue
        if event.event_type == StudentEvent.EVENT_STUDENT_DELETE_WORK_REQUEST and student_id not in latest_delete_request_by_student:
...
def build_upload_error_rows(
    *,
    classroom_id: int,
    students: list[StudentIdentity],
    material_lookup: dict[int, dict],
    upload_error_limit: int,
) -> list[dict]:
    student_by_id = {int(student.id): student for student in students}
    rows: list[dict] = []
...
def build_idle_rows(
    *,
    ...
    rows.sort(key=lambda row: (-int(row["idle_minutes"]), str(row["display_name"]).lower()))
    return rows[:idle_list_limit]
