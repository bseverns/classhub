...
class TeacherPortalHelperOpsTests(TeacherPortalBaseTests):
    ...
    def test_teach_class_dashboard_shows_remote_helper_compute_panel_for_superusers(self, status_mock, evidence_mock):
        ...
        self.assertContains(resp, "Classroom impact:")
        self.assertContains(resp, "What to do now:")
        self.assertContains(resp, "Trend state: <strong>Needs operator attention</strong>", html=False)
        self.assertContains(resp, "Local/default helper remains available while this path is reviewed.")
        self.assertContains(resp, "Provider reachability failed recently")
        self.assertContains(resp, "Operator Evidence And Exports")
