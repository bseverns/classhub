---
course: energy_electronics_circuits_9_session
session: 1
slug: s01-energy-is-everywhere
...
help:
  quick_fixes:
    ...
    - "If modules do not respond, reseat the magnetic connections and test one output at a time."
    - "If the build works once and stops, replace or recharge the power source."
extend:
  - "Label the build with arrows that show where energy starts and where it ends."
  - "Try one new output and compare what changed."
teacher_panel:
  purpose: "Set the tone that troubleshooting is part of making and that every session ends with a shareable artifact."
  snags:
    - "Students may call every part a battery or every action electricity."
    - "Students may snap modules in random order without isolating one working path."
  assessment:
    - "Student can point to one input and one output."
    - "Student can describe what changed when the build turned on."
...
- "It is not working."
  - Reconnect each module and test only one output.
- "I do not know what counts as energy."
  - Start with battery or power source, then name the light, sound, or motion it creates.

## Extensions
- Add a second output and explain whether both outputs happen at the same time.
- Compare two builds and decide which input feels easiest to control.
