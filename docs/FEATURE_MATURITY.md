...
| Telemetry DB split | RFC / staged plan | See `TELEMETRY_DB_SPLIT_PLAN.md` | Phase 1 Slice 0/1/2/3/4/5 scaffolding is shipped (flags + telemetry schema app + router + centralized dual-write seam + read abstraction on key teacher/operator rollups + idempotent backfill command); parity/cutover gates remain pending. |
