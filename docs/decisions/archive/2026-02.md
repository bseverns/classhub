...
## 2026-02-27 — Add stability freeze + survivability governance docs

**Why:**
- The repo had strong operational/security depth but needed explicit maintenance governance for a stabilization window.
- Staff turnover risk required a documented, repeatable operating posture that does not depend on one maintainer.

**Tradeoffs:**
- Adds process overhead (freeze rules, exception handling, recurring ritual expectations).
- Requires discipline to say no to feature expansion during the freeze period.

**Plan:**
- Add `STABILITY_CHARTER.md` with a 30-day freeze policy and reviewer checklist.
- Add `MAINTENANCE_RISK_REGISTER.md` with five evidence-backed long-term risks.
- Add `30_DAY_STABILITY_PLAN.md` with week-by-week stabilization tasks and acceptance criteria.
- Add `STAFF_TURNOVER_SURVIVABILITY.md` with maintainer-loss and onboarding/ritual guidance.
- Expose these docs in MkDocs navigation under Operations.

## 2026-02-27 — Add role-based human entry-point docs for operations and program staff

**Why:**
- Leadership, fundraising, and instructors needed faster role-specific onboarding paths without weakening existing security/runbook docs.
- Program-facing teams needed concise narrative and checklist docs tied to existing portal behavior.

**Tradeoffs:**
- More docs files to maintain.
- Requires active cross-link discipline to avoid conflicting guidance.

**Plan:**
- Add role-specific start pages:
  - `START_HERE_ED.md`
  - `START_HERE_OD.md`
  - `START_HERE_FUNDRAISING.md`
  - `START_HERE_INSTRUCTOR.md`
- Add operational narrative/checklist docs:
  - `RISK_AND_DATA_POSTURE.md`
  - `RUN_A_CLASS_TOMORROW.md`
  - `PROGRAM_LIFECYCLE.md`
  - `COMMON_SCENARIOS.md`
- Update `mkdocs.yml` and key landing pages to surface role-entry navigation.

## 2026-02-27 — Promote stability governance into active decision policy

**Why:**
- Stability and survivability guidance should be enforceable active policy, not only historical notes.
- Ops/maintainer behavior during turnover and freeze windows needs explicit decision ownership and cadence.

**Tradeoffs:**
- `DECISIONS.md` gets longer and more process-oriented.
- Requires periodic upkeep so governance sections stay aligned with reality.

**Plan:**
- Add active decision sections for:
  ...
  - outcomes/certificate semantics contract
  - accessibility runtime contract (`node`/`npm` prerequisites)
- Add these sections to `Active Decisions Snapshot` for discoverability.
...
- Document canonical usage + env knobs in `REQUEST_SAFETY.md`.
...
**Plan:**
- Add `WHAT_WHERE_WHY.md` with URL map, role map, quick checklist, and troubleshooting.
...
- Document the script in `DEVELOPMENT.md`.
